...
# select MTREC, year 1, corn>corn>corn>corn sequence
seqmet2 <- seqmet[which(seqmet$Location == "MTREC" & seqmet$Yr=="1" & seqmet$SequenceLet=="C>C>C>C"),]
...
# 2040 sequence variants were inferred from 39775 
...
# 171199 (this is the number of asvs)
...
## Identified 133822 bimeras out of 171199 input sequences.
dim(seqtab.nochim)
## [1]   16 37377
1-dim(seqtab.nochim)[2]/dim(seqtab)[2]
## 0.7816751: proportion of sequence variants that are not chimeric
1-sum(seqtab.nochim)/sum(seqtab)
## 0.2254322: proportion of sequences that are not chimeric
...
mean(track$final_perc_reads_retained) # 59.1% reads retained
...
out_M1 <- out 
seqmet2_M1cccc <- seqmet2
filtFs_M1cccc <- filtFs
filtRs_M1cccc <- filtRs
fnFs_M1cccc <- fnFs
fnRs_M1cccc <- fnRs
sample.names_M1cccc <- sample.names
errF_M1cccc <- errF
errR_M1cccc <- errR
derep_forward_M1cccc <- derep_forward
derep_reverse_M1cccc <- derep_reverse
dadaFs_M1cccc <- dadaFs
dadaRs_M1cccc <- dadaRs
mergers_M1cccc <- mergers
seqtab_M1cccc <- seqtab
seqtab.nochim_M1cccc <- seqtab.nochim
subset_M1cccc <- subset
...
path <- "Raw-data/study-sequences/Ashworth/ash fastq files"
subset <- "/mtrec-year-1-cccc"
write.csv(seqtab.nochim_M1cccc, file=paste0(path,subset, "/seqtabs/seqtab1.csv"))
save(list=ls(), file=paste0(path,"list-mtrec-year-1-cccc.RData"))
