...
# select MTREC, year 1, corn>soybean>corn>soybean sequence
seqmet2 <- seqmet[which(seqmet$Location == "MTREC" & seqmet$Yr=="1" & seqmet$SequenceLet=="C>S>C>S"),]
...
out_M1cscs <- out 
seqmet2_M1cscs <- seqmet2
filtFs_M1cscs <- filtFs
filtRs_M1cscs <- filtRs
fnFs_M1cscs <- fnFs
fnRs_M1cscs <- fnRs
sample.names_M1cscs <- sample.names
errF_M1cscs <- errF
errR_M1cscs <- errR
derep_forward_M1cscs <- derep_forward
derep_reverse_M1cscs <- derep_reverse
dadaFs_M1cscs <- dadaFs
dadaRs_M1cscs <- dadaRs
mergers_M1cscs <- mergers
seqtab_M1cscs <- seqtab
seqtab.nochim_M1cscs <- seqtab.nochim
subset_M1cscs <- subset
...
path <- "Raw-data/study-sequences/Ashworth/ash fastq files"
subset <- "/mtrec-year-1-cscs"
write.csv(seqtab.nochim_M1cscs, file=paste0(path,subset_M1cscs, "/seqtabs/seqtab1.csv"))
save(list=ls(), file=paste0(path,"list-mtrec-year-1-cscs.RData"))
