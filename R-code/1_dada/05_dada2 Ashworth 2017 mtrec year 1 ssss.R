...
# select MTREC, year 1, soybean>soybean>soybean>soybean sequence
seqmet2 <- seqmet[which(seqmet$Location == "MTREC" & seqmet$Yr=="1" & seqmet$SequenceLet=="S>S>S>S"),]
...
out_M1ssss <- out 
seqmet2_M1ssss <- seqmet2
filtFs_M1ssss <- filtFs
filtRs_M1ssss <- filtRs
fnFs_M1ssss <- fnFs
fnRs_M1ssss <- fnRs
sample.names_M1ssss <- sample.names
errF_M1ssss <- errF
errR_M1ssss <- errR
derep_forward_M1ssss <- derep_forward
derep_reverse_M1ssss <- derep_reverse
dadaFs_M1ssss <- dadaFs
dadaRs_M1ssss <- dadaRs
mergers_M1ssss <- mergers
seqtab_M1ssss <- seqtab
seqtab.nochim_M1ssss <- seqtab.nochim
subset_M1ssss <- subset
...
path <- "Raw-data/study-sequences/Ashworth/ash fastq files"
subset <- "/mtrec-year-1-ssss"
write.csv(seqtab.nochim_M1ssss, file=paste0(path,subset, "/seqtabs/seqtab1.csv"))
save(list=ls(), file=paste0(path,"list-mtrec-year-1-ssss.RData"))
