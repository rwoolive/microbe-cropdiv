...
# select MTREC, year 2, corn>corn>corn>corn sequence
seqmet2 <- seqmet[which(seqmet$Location == "MTREC" & seqmet$Yr=="2" & seqmet$SequenceLet=="C>C>C>C"),]
...
out_M2 <- out 
seqmet2_M2cccc <- seqmet2
filtFs_M2cccc <- filtFs
filtRs_M2cccc <- filtRs
fnFs_M2cccc <- fnFs
fnRs_M2cccc <- fnRs
sample.names_M2cccc <- sample.names
errF_M2cccc <- errF
errR_M2cccc <- errR
derep_forward_M2cccc <- derep_forward
derep_reverse_M2cccc <- derep_reverse
dadaFs_M2cccc <- dadaFs
dadaRs_M2cccc <- dadaRs
mergers_M2cccc <- mergers
seqtab_M2cccc <- seqtab
seqtab.nochim_M2cccc <- seqtab.nochim
subset_M2cccc <- subset
...
# bacteria
subset_M2cccc <- t(subset_M2cccc)
path <- "Raw-data/study-sequences/Ashworth/ash fastq files"
subset <- "/mtrec-year-2-cccc"
write.csv(subset_M2cccc, file=paste0(path,subset, "/seqtabs/seqtab1.csv"))
save(list=ls(), file=paste0(path,"list-mtrec-year-2-cccc.RData"))
