...
# select MTREC, year 2, corn>soybean>corn>soybean sequence
seqmet2 <- seqmet[which(seqmet$Location == "MTREC" & seqmet$Yr=="2" & seqmet$SequenceLet=="C>S>C>S"),]
...
out_M2cscs <- out 
seqmet2_M2cscs <- seqmet2
filtFs_M2cscs <- filtFs
filtRs_M2cscs <- filtRs
fnFs_M2cscs <- fnFs
fnRs_M2cscs <- fnRs
sample.names_M2cscs <- sample.names
errF_M2cscs <- errF
errR_M2cscs <- errR
derep_forward_M2cscs <- derep_forward
derep_reverse_M2cscs <- derep_reverse
dadaFs_M2cscs <- dadaFs
dadaRs_M2cscs <- dadaRs
mergers_M2cscs <- mergers
seqtab_M2cscs <- seqtab
seqtab.nochim_M2cscs <- seqtab.nochim
subset_M2cscs <- subset
...
path <- "Raw-data/study-sequences/Ashworth/ash fastq files"
subset <- "/mtrec-year-2-cscs"
write.csv(seqtab.nochim_M2cscs, file=paste0(path,subset, "/seqtabs/seqtab1.csv"))
save(list=ls(), file=paste0(path,"list-mtrec-year-2-cscs.RData"))
