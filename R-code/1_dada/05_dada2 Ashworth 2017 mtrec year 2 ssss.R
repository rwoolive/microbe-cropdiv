...
# select MTREC, year 2, soybean>soybean>soybean>soybean sequence
seqmet2 <- seqmet[which(seqmet$Location == "MTREC" & seqmet$Yr=="2" & seqmet$SequenceLet=="S>S>S>S"),]
...
out_M2ssss <- out 
seqmet2_M2ssss <- seqmet2
filtFs_M2ssss <- filtFs
filtRs_M2ssss <- filtRs
fnFs_M2ssss <- fnFs
fnRs_M2ssss <- fnRs
sample.names_M2ssss <- sample.names
errF_M2ssss <- errF
errR_M2ssss <- errR
derep_forward_M2ssss <- derep_forward
derep_reverse_M2ssss <- derep_reverse
dadaFs_M2ssss <- dadaFs
dadaRs_M2ssss <- dadaRs
mergers_M2ssss <- mergers
seqtab_M2ssss <- seqtab
seqtab.nochim_M2ssss <- seqtab.nochim
subset_M2ssss <- subset
...
path <- "Raw-data/study-sequences/Ashworth/ash fastq files"
subset <- "/mtrec-year-2-ssss"
write.csv(seqtab.nochim_M2ssss, file=paste0(path,subset, "/seqtabs/seqtab1.csv"))
save(list=ls(), file=paste0(path,"list-mtrec-year-2-ssss.RData"))
