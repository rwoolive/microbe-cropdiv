...
# select RECM, year 1, corn>corn>corn>corn sequence
seqmet2 <- seqmet[which(seqmet$Location == "RECM" & seqmet$Yr=="1" & seqmet$SequenceLet=="C>C>C>C"),]
...
out_R1 <- out 
seqmet2_R1cccc <- seqmet2
filtFs_R1cccc <- filtFs
filtRs_R1cccc <- filtRs
fnFs_R1cccc <- fnFs
fnRs_R1cccc <- fnRs
sample.names_R1cccc <- sample.names
errF_R1cccc <- errF
errR_R1cccc <- errR
derep_forward_R1cccc <- derep_forward
derep_reverse_R1cccc <- derep_reverse
dadaFs_R1cccc <- dadaFs
dadaRs_R1cccc <- dadaRs
mergers_R1cccc <- mergers
seqtab_R1cccc <- seqtab
seqtab.nochim_R1cccc <- seqtab.nochim
subset_R1cccc <- subset
...
path <- "Raw-data/study-sequences/Ashworth/ash fastq files"
subset <- "/recm-year-1-cccc"
write.csv(seqtab.nochim_R1cccc, file=paste0(path,subset, "/seqtabs/seqtab1.csv"))
save(list=ls(), file=paste0(path,"list-recm-year-1-cccc.RData"))
