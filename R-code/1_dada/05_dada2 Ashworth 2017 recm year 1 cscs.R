...
# select recm, year 1, corn>soybean>corn>soybean sequence
seqmet2 <- seqmet[which(seqmet$Location == "RECM" & seqmet$Yr=="1" & seqmet$SequenceLet=="C>S>C>S"),]
...
out_R1cscs <- out 
seqmet2_R1cscs <- seqmet2
filtFs_R1cscs <- filtFs
filtRs_R1cscs <- filtRs
fnFs_R1cscs <- fnFs
fnRs_R1cscs <- fnRs
sample.names_R1cscs <- sample.names
errF_R1cscs <- errF
errR_R1cscs <- errR
derep_forward_R1cscs <- derep_forward
derep_reverse_R1cscs <- derep_reverse
dadaFs_R1cscs <- dadaFs
dadaRs_R1cscs <- dadaRs
mergers_R1cscs <- mergers
seqtab_R1cscs <- seqtab
seqtab.nochim_R1cscs <- seqtab.nochim
subset_R1cscs <- subset
...
path <- "Raw-data/study-sequences/Ashworth/ash fastq files"
subset <- "/recm-year-1-cscs"
write.csv(seqtab.nochim_R1cscs, file=paste0(path,subset, "/seqtabs/seqtab1.csv"))
save(list=ls(), file=paste0(path,"list-recm-year-1-cscs.RData"))
