...
table(seqmet$Yr, seqmet$Location, seqmet$SequenceLet)
# select RECM, year 1, cotton>cotton>cotton>cotton sequence
seqmet2 <- seqmet[which(seqmet$Location == "RECM" & seqmet$Yr=="1" & seqmet$SequenceLet=="Ct>Ct>Ct>Ct"),]
...
out_R2ctctctct <- out 
seqmet2_R2ctctctct <- seqmet2
filtFs_R2ctctctct <- filtFs
filtRs_R2ctctctct <- filtRs
fnFs_R2ctctctct <- fnFs
fnRs_R2ctctctct <- fnRs
sample.names_R2ctctctct <- sample.names
errF_R2ctctctct <- errF
errR_R2ctctctct <- errR
derep_forward_R2ctctctct <- derep_forward
derep_reverse_R2ctctctct <- derep_reverse
dadaFs_R2ctctctct <- dadaFs
dadaRs_R2ctctctct <- dadaRs
mergers_R2ctctctct <- mergers
seqtab_R2ctctctct <- seqtab
seqtab.nochim_R2ctctctct <- seqtab.nochim
subset_R2ctctctct <- subset
...
path <- "Raw-data/study-sequences/Ashworth/ash fastq files"
subset <- "/recm-year-1-ctctctct"
write.csv(seqtab.nochim_R2ctctctct, file=paste0(path,subset, "/seqtabs/seqtab1.csv"))
save(list=ls(), file=paste0(path,"list-recm-year-1-ctctctct.RData"))
