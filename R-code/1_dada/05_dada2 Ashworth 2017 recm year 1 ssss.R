...
# select RECM, year 1, soybean>soybean>soybean>soybean sequence
seqmet2 <- seqmet[which(seqmet$Location == "RECM" & seqmet$Yr=="1" & seqmet$SequenceLet=="S>S>S>S"),]
...
out_R1ssss <- out 
seqmet2_R1ssss <- seqmet2
filtFs_R1ssss <- filtFs
filtRs_R1ssss <- filtRs
fnFs_R1ssss <- fnFs
fnRs_R1ssss <- fnRs
sample.names_R1ssss <- sample.names
errF_R1ssss <- errF
errR_R1ssss <- errR
derep_forward_R1ssss <- derep_forward
derep_reverse_R1ssss <- derep_reverse
dadaFs_R1ssss <- dadaFs
dadaRs_R1ssss <- dadaRs
mergers_R1ssss <- mergers
seqtab_R1ssss <- seqtab
seqtab.nochim_R1ssss <- seqtab.nochim
subset_R1ssss <- subset
...
path <- "Raw-data/study-sequences/Ashworth/ash fastq files"
subset <- "/recm-year-1-ssss"
write.csv(seqtab.nochim_R1ssss, file=paste0(path,subset, "/seqtabs/seqtab1.csv"))
save(list=ls(), file=paste0(path,"list-recm-year-1-ssss.RData"))
