...
# select RECM, year 2, corn>soybean>corn>soybean sequence
seqmet2 <- seqmet[which(seqmet$Location == "RECM" & seqmet$Yr=="2" & seqmet$SequenceLet=="C>S>C>S"),]
...
out_R2cscs <- out 
seqmet2_R2cscs <- seqmet2
filtFs_R2cscs <- filtFs
filtRs_R2cscs <- filtRs
fnFs_R2cscs <- fnFs
fnRs_R2cscs <- fnRs
sample.names_R2cscs <- sample.names
errF_R2cscs <- errF
errR_R2cscs <- errR
derep_forward_R2cscs <- derep_forward
derep_reverse_R2cscs <- derep_reverse
dadaFs_R2cscs <- dadaFs
dadaRs_R2cscs <- dadaRs
mergers_R2cscs <- mergers
seqtab_R2cscs <- seqtab
seqtab.nochim_R2cscs <- seqtab.nochim
subset_R2cscs <- subset
...
path <- "Raw-data/study-sequences/Ashworth/ash fastq files"
subset <- "/recm-year-2-cscs"
write.csv(seqtab.nochim_R2cscs, file=paste0(path,subset, "/seqtabs/seqtab1.csv"))
save(list=ls(), file=paste0(path,"list-recm-year-2-cscs.RData"))
