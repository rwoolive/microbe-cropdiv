...
# select RECM, year 2, soybean>soybean>soybean>soybean sequence
seqmet2 <- seqmet[which(seqmet$Location == "RECM" & seqmet$Yr=="2" & seqmet$SequenceLet=="S>S>S>S"),]
...
out_R2ssss <- out 
seqmet2_R2ssss <- seqmet2
filtFs_R2ssss <- filtFs
filtRs_R2ssss <- filtRs
fnFs_R2ssss <- fnFs
fnRs_R2ssss <- fnRs
sample.names_R2ssss <- sample.names
errF_R2ssss <- errF
errR_R2ssss <- errR
derep_forward_R2ssss <- derep_forward
derep_reverse_R2ssss <- derep_reverse
dadaFs_R2ssss <- dadaFs
dadaRs_R2ssss <- dadaRs
mergers_R2ssss <- mergers
seqtab_R2ssss <- seqtab
seqtab.nochim_R2ssss <- seqtab.nochim
subset_R2ssss <- subset
...
path <- "Raw-data/study-sequences/Ashworth/ash fastq files"
subset <- "/recm-year-2-ssss"
write.csv(seqtab.nochim_R2ssss, file=paste0(path,subset, "/seqtabs/seqtab1.csv"))
save(list=ls(), file=paste0(path,"list-recm-year-2-ssss.RData"))
