...
path <- "Raw-data/study-sequences/Strom-2020/seqs"
...
sumabund <- colSums(abundances[,3:50])

# columns for plots in taxabund
taxabund <- merge(taxa, abundances, by.x = "X.2", by.y = "X.1", all.x = TRUE, all.y = FALSE) 
plots <- 22:117
...
for(i in 1:10){
  taxabund <- merge(taxa[which(taxa$Primary_lifestyle==d$group[i]),], abundances, by.x = "X.2", by.y = "X.1", all.x = TRUE, all.y = FALSE)
...
for(i in 11:15){
  taxabund <- merge(taxa[taxa$Phylum==d$group[i],], abundances, by.x = "X.2", by.y = "X.1", all.x = TRUE, all.y = FALSE) 
  d$avabund.percent[i] <-  round(mean(colSums(taxabund[,plots])/sumabund),3)*100
}
