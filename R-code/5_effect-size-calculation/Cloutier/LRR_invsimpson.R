...
meta$X
divdat <- read.csv("Processed-data/Cloutier-diversity.csv")
divdat$Season_num <- divdat$Season
divdat$Season_num <- as.factor(dplyr::recode(divdat$Season_num, Spring = "1", Summer = "2"))
divdat$Cover2 <- divdat$Cover
divdat$Cover2 <- as.factor(dplyr::recode(divdat$Cover2, "3 Spp mix" = "3SppN", "6 Spp mix" = "6Spp"))
...
# mixed effects model
fit <- lme(invsimpson.div ~ Cover*Season, random = ~1|Replicate, data = divdat)
# check redisuals
resid <- residuals(fit)
...
p <- ggplot(divdat, aes(x=Cover, y=invsimpson.div, fill=Cover)) +
  ...
  labs(x="Cover", y=expression(paste("Fungal Inverse Simpson diversity"))) +
  lims(y=c(range(na.omit(divdat$invsimpson.div)) + c(0,80))) +
  geom_jitter(alpha=0.75, width=0.2, height=0, shape=19, size=3) +
  theme_bw() +
  scale_fill_manual(values=colorvec) +
  guides(fill=FALSE) +
  geom_text(x = c(1:9), y = rep(920, 9), aes(label = .group), data = testlet)
p
ggpubr::ggexport(p, height=3, width=7, filename = "Figures/effect-plots/Cloutier_fun_cover.pdf")



write.csv(t(mod.effects), "Model-output/effects_diversity/Cloutier_diversity_anova.csv")
...
write.csv(d, "Model-output/effects_diversity/Cloutier_diversity_RR.csv")
...
hist((divdat$SOC)^3)
# mixed effects model
fit <- lme((SOC)^3 ~ Cover*Season, random = ~1|Replicate, data = divdat)
# check redisuals
resid <- residuals(fit)
qqnorm(resid); qqline(resid); shapiro.test(resid)
# check variances
...
# compact letter display
testlet <- cld(test1, type = "response", Letters = "ABCD", reversed=T)
testlet_SOC <- testlet[order(testlet$Cover),]
p <- ggplot(divdat, aes(x=Cover, y=SOC, fill=Cover)) +
  geom_boxplot(alpha=0.5, outlier.alpha = 0) +
  ...
  scale_fill_manual(values=colorvec) +
  guides(fill=FALSE) +
  geom_text(x = c(1:9), y = rep(2.75, 9), aes(label = .group), data = testlet_SOC)
p
ggpubr::ggexport(p, height=4, width=7, filename = "Figures/effect-plots/Cloutier_SOC_cover.pdf")
...
Lt <- "Canola" # label for diversity treatment
Lc <- "Fallow" # label for diversity control
Yt <- divdat$SOC[which(divdat$Cover==Lt)] # diversity treatment mean
Yc <- divdat$SOC[which(divdat$Cover==Lc)] # diversity control mean
...
write.csv(d, "Model-output/effects_diversity/Cloutier_carbon-SOC_RR.csv")
...
lmtest_OM <- lme(scale(OM) ~ scale(invsimpson.div), random = ~1|Replicate, data = divdat, na.action="na.omit")
# check redisuals
resid <- residuals(lmtest_OM)
qqnorm(resid); qqline(resid); shapiro.test(resid)
lmtest_OM_sum <- as.data.frame(summary(lmtest_OM)$tTable)
...
df <- data.frame(statistic = c("intercept", "slope", "t", "df", "p"),
                 ...
                 value_OM = c(lmtest_OM_sum$Value[1], lmtest_OM_sum$Value[2], lmtest_OM_sum$`t-value`[2], lmtest_OM_sum$DF[2], lmtest_OM_sum$`p-value`[2]))


write.csv(df, "Model-output/effects_diversity/Cloutier_lmm-carbon.csv")




p <- ggplot(divdat, aes(x=invsimpson.div, y=POXC)) +
  geom_line(aes(y=predlmPOXC), size=1, color="black", linetype="dashed") +
  labs(x="Bacterial Inverse Simpson diversity", y=expression(paste("Permanganate oxidizable carbon (mg kg"^-1,")")), fill="Cropping system", subtitle="C. Cloutier et al. (2020) POX-C") +
  geom_point(aes(fill=Cover, shape=Season)) +
  scale_shape_manual(values=c(21,23)) +
  theme_bw()  + 
  guides(fill = guide_legend(override.aes=list(shape=21))) 
p
ggpubr::ggexport(p, height=3.5, width=6, filename = "Figures/regressions/Cloutier_poxc-invsimpson.div_fun.pdf")


p <- ggplot(divdat, aes(x=invsimpson.div, y=SOC)) +
  geom_line(aes(y=predlmSOC), size=1, color="black", linetype="dashed") +
  labs(x="Bacterial Inverse Simpson diversity", y=expression(paste("Soil organic carbon (%)")), fill="Cropping system", subtitle="D. Cloutier et al. (2020) SOC") +
  geom_point(aes(fill=Cover, shape=Season)) +
  scale_shape_manual(values=c(21,23)) +
  theme_bw()  + 
  guides(fill = guide_legend(override.aes=list(shape=21))) 
p
ggpubr::ggexport(p, height=3.5, width=6, filename = "Figures/regressions/Cloutier_SOC-invsimpson.div_fun.pdf")
