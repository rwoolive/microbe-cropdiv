...
meta$X
divdat <- read.csv("Processed-data/Cloutier-diversity_pathogen.csv")
...
p <- ggplot(divdat, aes(x=Cover, y=invsimpson.div, fill=Cover)) +
  ...
  labs(x="Cover", y=expression(paste("Pathogen Inverse Simpson diversity"))) +
  lims(y=c(range(na.omit(divdat$invsimpson.div)) + c(0,10))) +
  geom_jitter(alpha=0.75, width=0.2, height=0, shape=19, size=3) +
  theme_bw() +
  scale_fill_manual(values=colorvec) +
  guides(fill=FALSE) +
  geom_text(x = c(1:9), y = rep(135, 9), aes(label = .group), data = testlet)
p
ggpubr::ggexport(p, height=3, width=7, filename = "Figures/effect-plots/Cloutier_fun_pathogen_cover.pdf")

# visualize
### tukey & visualize
# compute least-squares means
test1 <- emmeans(fit, ~Season)
# compact letter display
testlet <- cld(test1, type = "response", Letters = "ABCD", reversed=T)
testlet <- testlet[order(testlet$Season),]
p <- ggplot(divdat, aes(x=Season, y=invsimpson.div, fill=Season)) +
  geom_boxplot(alpha=0.5, outlier.alpha = 0) +
  labs(x="Season", y=expression(paste("Pathogen Inverse Simpson diversity"))) +
  lims(y=c(range(na.omit(divdat$invsimpson.div)) + c(0,10))) +
  geom_jitter(alpha=0.75, width=0.2, height=0, shape=19, size=3) +
  theme_bw() +
  guides(fill=FALSE) +
  geom_text(x = c(1:2), y = rep(135, 2), aes(label = .group), data = testlet)
p
ggpubr::ggexport(p, height=3, width=7, filename = "Figures/effect-plots/Cloutier_fun_pathogen_Season.pdf")



write.csv(t(mod.effects), "Model-output/effects_diversity/Cloutier_diversity_pathogen_anova.csv")
...
write.csv(d, "Model-output/effects_diversity/Cloutier_diversity_pathogen_RR.csv")
...
write.csv(df, "Model-output/effects_diversity/Cloutier_lmm-carbon_pathogen.csv")
...
p <- ggplot(divdat, aes(x=invsimpson.div, y=OM)) +
  geom_smooth(method=lm, se=TRUE, fullrange=FALSE, color="black", linetype="solid") +
  labs(x="pathogen Inverse Simpson diversity", y=expression(paste("Soil organic matter (%)")), fill="Cropping system") +
...
q <- ggplot(divdat, aes(x=invsimpson.div, y=POXC)) +
  ...
  labs(x="pathogen Inverse Simpson diversity", y=expression(paste("Permanganate oxidizable carbon (mg kg"^-1,")")), fill="Cropping system") +
  geom_point(size = 3, shape=21, aes(fill=Cover)) +
  theme_bw()  +
...
s <- ggpubr::annotate_figure(r, bottom = ggpubr::text_grob("pathogen Inverse Simpson diversity"))
ggpubr::ggexport(s, filename="Figures/regressions/Cloutier-invsimpson.div_fung_pathogen.pdf", height=3.25,width=8)
