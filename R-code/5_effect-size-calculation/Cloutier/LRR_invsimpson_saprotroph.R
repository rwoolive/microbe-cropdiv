...
meta$X
divdat <- read.csv("Processed-data/Cloutier-diversity_all-saprotrophs.csv")
...
p <- ggplot(divdat, aes(x=Cover, y=invsimpson.div, fill=Cover)) +
  ...
  labs(x="Cover", y=expression(paste("saprotroph Inverse Simpson diversity"))) +
  lims(y=c(range(na.omit(divdat$invsimpson.div)) + c(0,30))) +
  geom_jitter(alpha=0.75, width=0.2, height=0, shape=19, size=3) +
  theme_bw() +
  scale_fill_manual(values=colorvec) +
  guides(fill=FALSE) +
  geom_text(x = c(1:9), y = rep(390, 9), aes(label = .group), data = testlet)
p
ggpubr::ggexport(p, height=3, width=7, filename = "Figures/effect-plots/Cloutier_fun_saprotroph_cover.pdf")
...
p <- ggplot(divdat, aes(x=Season, y=invsimpson.div, fill=Season)) +
  geom_boxplot(alpha=0.5, outlier.alpha = 0) +
  labs(x="Season", y=expression(paste("saprotroph Inverse Simpson diversity"))) +
  lims(y=c(range(na.omit(divdat$invsimpson.div)) + c(0,30))) +
  geom_jitter(alpha=0.75, width=0.2, height=0, shape=19, size=3) +
  theme_bw() +
  guides(fill=FALSE) +
  geom_text(x = c(1:2), y = rep(390, 2), aes(label = .group), data = testlet)
p
ggpubr::ggexport(p, height=3, width=7, filename = "Figures/effect-plots/Cloutier_fun_saprotroph_Season.pdf")



write.csv(t(mod.effects), "Model-output/effects_diversity/Cloutier_diversity_saprotroph_anova.csv")
...
write.csv(d, "Model-output/effects_diversity/Cloutier_diversity_saprotroph_RR.csv")
...
write.csv(df, "Model-output/effects_diversity/Cloutier_lmm-carbon_saprotroph.csv")
...
p <- ggplot(divdat, aes(x=invsimpson.div, y=OM)) +
  ...
  labs(x="saprotroph Inverse Simpson diversity", y=expression(paste("Soil organic matter (%)")), fill="Cropping system") +
...
q <- ggplot(divdat, aes(x=invsimpson.div, y=POXC)) +
  geom_smooth(method=lm, se=TRUE, fullrange=FALSE, color="black", linetype="dashed") +
  labs(x="saprotroph Inverse Simpson diversity", y=expression(paste("Permanganate oxidizable carbon (mg kg"^-1,")")), fill="Cropping system") +
  geom_point(size = 3, shape=21, aes(fill=Cover)) +
  theme_bw()  +
...
s <- ggpubr::annotate_figure(r, bottom = ggpubr::text_grob("saprotroph Inverse Simpson diversity"))
ggpubr::ggexport(s, filename="Figures/regressions/Cloutier-invsimpson.div_fung_saprotroph.pdf", height=3.25,width=8)
