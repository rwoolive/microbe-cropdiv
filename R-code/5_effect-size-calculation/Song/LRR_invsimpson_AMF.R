...
divdat_fun <- read.csv("Processed-data/Song-diversity_fun_AMF.csv")
...
p <- ggplot(dat_fun, aes(x=Cropping.system, y=invsimpson.div, fill=Cropping.system)) +
  ...
  scale_fill_manual(values=colorvec) +
  guides(fill=FALSE) +
  geom_text(x = c(1,2,3,4), y = rep(27, 4), aes(label = .group), data = testlet)
p
ggpubr::ggexport(p, height=3, width=5, filename = "Figures/effect-plots/Song_fun_AMF_cropsys.pdf")



write.csv(t(mod.effects), "Model-output/effects_diversity/Song_fun_AMF-diversity_anova.csv")
...
write.csv(d, "Model-output/effects_diversity/Song_fun_AMF-diversity_RR.csv")
...
write.csv(df, "Model-output/effects_diversity/Song_lmm-carbon_fun_amf.csv")
...
r <- ggplot(dat_fun, aes(x=invsimpson.div, y=SOM)) +
  ...
  labs(x="AMF Inverse Simpson diversity", y=expression(paste("Soil organic matter (g kg"^-1, ")")),
...
ggpubr::ggexport(s, filename="Figures/regressions/Song_som-invsimpson.div_fun_AMF.pdf", height=3.25,width=5)
