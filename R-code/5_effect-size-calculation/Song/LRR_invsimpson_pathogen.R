...
divdat_fun <- read.csv("Processed-data/Song-diversity_fun_plant-pathogens.csv")
...
p <- ggplot(dat_fun, aes(x=Cropping.system, y=invsimpson.div, fill=Cropping.system)) +
  ...
  lims(y=c(range(na.omit(dat_fun$invsimpson.div)) + c(0,2))) +
  geom_jitter(alpha=0.75, width=0.2, height=0, shape=19, size=3) +
  theme_bw() +
  scale_fill_manual(values=colorvec) +
  guides(fill=FALSE) +
...
p
ggpubr::ggexport(p, height=3, width=5, filename = "Figures/effect-plots/Song_fun_pathogen_cropsys.pdf")



write.csv(t(mod.effects), "Model-output/effects_diversity/Song_fun_pathogen-diversity_anova.csv")
...
write.csv(d, "Model-output/effects_diversity/Song_fun_pathogen-diversity_RR.csv")
...
write.csv(df, "Model-output/effects_diversity/Song_lmm-carbon_fun_pathogen.csv")
...
r <- ggplot(dat_fun, aes(x=invsimpson.div, y=SOM)) +
  ...
  labs(x="pathogen Inverse Simpson diversity", y=expression(paste("Soil organic matter (g kg"^-1, ")")),
...
ggpubr::ggexport(s, filename="Figures/regressions/Song_som-invsimpson.div_fun_pathogen.pdf", height=3.25,width=5)
