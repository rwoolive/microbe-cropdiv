...
divdat <- read.csv( "Processed-data/Strom-diversity_AMF.csv")
...
testlet <- testlet[order(testlet$Cropping.system, testlet$Year),] # no differences
p <- ggplot(divdat, aes(x=Cropping.system, y=invsimpson.div, fill=Cropping.system)) +
  geom_boxplot(alpha=0.5, outlier.alpha = 0) +
  labs(x="Cropping system", y=expression(paste("AMF Inverse Simpson diversity"))) +
  ...
  guides(fill=FALSE) +
  geom_text(x = rep(c(1,2,3,4), 2), y = rep(45, 8), aes(label = .group), data = testlet)
p
ggpubr::ggexport(p, height=3, width=5, filename = "Figures/effect-plots/Strom_fun_AMF_cropsys.pdf")
...
write.csv(t(mod.effects), "Model-output/effects_diversity/Strom_AMF_diversity_anova.csv")
...
write.csv(d, "Model-output/effects_diversity/Strom_AMF_diversity_RR.csv")
...
write.csv(df, "Model-output/effects_diversity/Strom_lmm-carbon_amf.csv")



p <- ggplot(divdat, aes(x=invsimpson.div, y=SOC)) +
  geom_smooth(method=lm, se=TRUE, fullrange=FALSE, color="black", linetype="dashed") +
  labs(x="AMF Inverse Simpson diversity", y=expression(paste("Soil organic carbon (%)")), fill="Cropping system") +
...
p
ggpubr::ggexport(p, height=3.2, width=4.75, filename = "Figures/regressions/Strom_SOC-invsimpson.div_fun_AMF.pdf")
