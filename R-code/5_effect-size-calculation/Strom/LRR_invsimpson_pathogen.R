...
divdat <- read.csv( "Processed-data/Strom-diversity_plant-pathogen.csv")
...
# mixed effects model
fit <- lme(sqrt(invsimpson.div) ~ Cropping.system*Year*Season, random = ~1|Replicate, data = divdat)
# check redisuals
resid <- residuals(fit)
...
test1 <- emmeans(fit, ~Cropping.system, by="Season")
# compact letter display
testlet <- cld(test1, type = "response", Letters = "ABCD", reversed=T)
testlet <- testlet[order(testlet$Season, testlet$Cropping.system),] # no differences
p <- ggplot(divdat, aes(x=Cropping.system, y=invsimpson.div, fill=Cropping.system)) +
  geom_boxplot(alpha=0.5, outlier.alpha = 0) +
  labs(x="Cropping system", y=expression(paste("pathogen Inverse Simpson diversity"))) +
  lims(y=c(range(na.omit(divdat$invsimpson.div)) + c(0,5))) +
  geom_jitter(alpha=0.75, width=0.2, height=0, shape=19, size=3) +
  theme_bw() +
  facet_wrap(.~Season) +
  scale_fill_manual(values=colorvec) +
  guides(fill=FALSE) +
  geom_text(x = rep(c(1,2,3,4),3), y = rep(21, 12), aes(label = .group), data = testlet)
p
ggpubr::ggexport(p, height=3, width=12, filename = "Figures/effect-plots/Strom_fun_pathogen_cropsys_season.pdf")
...
p <- ggplot(divdat, aes(x=Cropping.system, y=invsimpson.div, fill=Cropping.system)) +
  ...
  scale_fill_manual(values=colorvec) +
  guides(fill=FALSE) +
  geom_text(x = c(1,2,3,4), y = rep(21, 4), aes(label = .group), data = testlet)
p
ggpubr::ggexport(p, height=3, width=5, filename = "Figures/effect-plots/Strom_fun_pathogen_cropsys.pdf")



write.csv(t(mod.effects), "Model-output/effects_diversity/Strom_pathogen_diversity_anova.csv")
...
seas <- "Spring"
Yt <- divdat$invsimpson.div[which(divdat$Cropping.system==Lt & divdat$Season==seas)] # diversity treatment mean
Yc <- divdat$invsimpson.div[which(divdat$Cropping.system==Lc & divdat$Season==seas)] # diversity control mean

d$Comparison[comp] <- paste(Lt, Lc, sep=" vs. ")
d$Season[comp] <- seas
d[comp,3:7] <- LRRi(Yc, Yt, confidence = 0.95)
...
write.csv(d, "Model-output/effects_diversity/Strom_pathogen_diversity_RR.csv")
...
write.csv(df, "Model-output/effects_diversity/Strom_lmm-carbon_pathogen.csv")
...
p <- ggplot(divdat, aes(x=invsimpson.div, y=SOC)) +
  ...
  labs(x="pathogen Inverse Simpson diversity", y=expression(paste("Soil organic carbon (%)")), fill="Cropping system") +
...
p
ggpubr::ggexport(p, height=3.2, width=4.75, filename = "Figures/regressions/Strom_SOC-invsimpson.div_fun_pathogen.pdf")
