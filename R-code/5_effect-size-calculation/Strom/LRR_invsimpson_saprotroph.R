...
divdat <- read.csv( "Processed-data/Strom-diversity_all-saprotroph.csv")
...
p <- ggplot(divdat, aes(x=Cropping.system, y=invsimpson.div, fill=Cropping.system)) +
  ...
  guides(fill=FALSE) +
  geom_text(x = c(1,2,3,4), y = rep(45, 4), aes(label = .group), data = testlet)
p
ggpubr::ggexport(p, height=3, width=5, filename = "Figures/effect-plots/Strom_fun_saprotroph_cropsys.pdf")



write.csv(t(mod.effects), "Model-output/effects_diversity/Strom_saprotroph_diversity_anova.csv")
...
write.csv(d, "Model-output/effects_diversity/Strom_saprotroph_diversity_RR.csv")
...
write.csv(df, "Model-output/effects_diversity/Strom_lmm-carbon_saprotroph.csv")



p <- ggplot(divdat, aes(x=invsimpson.div, y=SOC)) +
  geom_smooth(method=lm, se=TRUE, fullrange=FALSE, color="black", linetype="dashed") +
  labs(x="saprotroph Inverse Simpson diversity", y=expression(paste("Soil organic carbon (%)")), fill="Cropping system") +
...
p
ggpubr::ggexport(p, height=3.2, width=4.75, filename = "Figures/regressions/Strom_SOC-invsimpson.div_fun_saprotroph.pdf")
